...
! Revision Date  920304   (YYMMDD)
   ...
   ! N.B. the locations of WD and TT accessed depend on the value of the first element of each
   ! array and the leading dimensions of the multiply subscripted arrays.
...
! References  Dongarra J.J., Bunch J.R., Moler C.B., Stewart G.W., *LINPACK Users Guide*, SIAM, 1979.
   ...
   use odrpack_kinds, only: wp, zero, ten
...
! Revision Date  920619   (YYMMDD)
   ...
   real(kind=wp), intent(in) :: beta(np)
      !! The function parameters.
   real(kind=wp), intent(in) :: betaj(np)
...
! Revision Date  920619   (YYMMDD)
   ...
   real(kind=wp), intent(out) :: fd !also in?
