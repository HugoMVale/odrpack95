...
contains
   ...
   !! Test solutions delivered with [[odr]].
      ...
      ! Parameters
      real(dp), parameter :: base = radix(1.0_dp)
      ...
      real(dp) :: bnrm, epsmac, ewrt, ewrt2, hundrd, one, p01, p2, partol, sstol, &
      ...
      real(dp) :: dpymp(2, ntests)
      real(dp), allocatable :: delta(:, :), beta(:), x(:, :), y(:, :), sclb(:), scld(:, :), &
      ...
      data &
         ...
         /0.0E0_dp, 0.01E0_dp, 0.2E0_dp, 1.0E0_dp, 2.0E0_dp, 3.0E0_dp, &
         100.0E0_dp/
      data &
         (dpymp(i, 1), i=1, 2) &
         /2.762733195780256808978449342964E+04_dp, &
         7.532639569022918943695104672512E-04_dp/
      data &
         (dpymp(i, 2), i=1, 2) &
         /2.762732630143673024399942947263E+04_dp, &
         7.538467722687131506874279314940E-04_dp/
      data &
         (dpymp(i, 3), i=1, 2) &
         /1.069944100000000027940905194068E+09_dp, &
         1.212808593256056359629660672046E-05_dp/
      data &
         (dpymp(i, 4), i=1, 2) &
         /1.069944100000000026623461142867E+09_dp, &
         5.452084633790606017572015067556E-07_dp/
      data &
         (dpymp(i, 5), i=1, 2) &
         /1.426988156377258617521571734503E+00_dp, &
         1.084728687127432219753903919409E+00_dp/
      data &
         (dpymp(i, 6), i=1, 2) &
         /4.261321829513978871872508874025E+00_dp, &
         1.477967210398420733565424329280E-02_dp/
      data &
         (dpymp(i, 7), i=1, 2) &
         /4.261272307142888464011486769858E+00_dp, &
         1.477966125465374336804138554559E-02_dp/
      data &
         (dpymp(i, 8), i=1, 2) &
         /4.371487317909745009110272283622E+01_dp, &
         1.144419474408286067112233592550E-03_dp/
      data &
         (dpymp(i, 9), i=1, 2) &
         /3.099048849376848610380977303924E+00_dp, &
         8.824708863783850023783338218501E-02_dp/
      data &
         (dpymp(i, 10), i=1, 2) &
         /9.469917836739932584221023234527E+00_dp, &
         4.205389215588104651198536809880E-01_dp/
      data &
         (dpymp(i, 11), i=1, 2) &
         /3.950949253027682207109233363651E+01_dp, &
         6.651838750834910819636881506915E+01_dp/
      data &
         (dpymp(i, 12), i=1, 2) &
         /3.950949253027682207109233363651E+01_dp, &
         6.651838750834910819636881506915E+01_dp/
      data &
         (dpymp(i, 13), i=1, 2) &
         /1.414213562373095000000000000000E+00_dp, &
         5.250825926608277346013642256883E-26_dp/
      data &
         (dpymp(i, 14), i=1, 2) &
         /1.414213562373095000000000000000E+00_dp, &
         8.159081600696301507018019048968E-26_dp/
      data &
         (dpymp(i, 15), i=1, 2) &
         /1.486588477064952451556223422813E+00_dp, &
         1.841690442255357083922717720270E+03_dp/
      data &
         (dpymp(i, 16), i=1, 2) &
         /2.001224625073357401561224833131E+02_dp, &
         0.000000000000000000000000000000E+00_dp/
      data &
         (dpymp(i, 17), i=1, 2) &
         /2.000099997500125000000000000000E+02_dp, &
         0.000000000000000000000000000000E+00_dp/
      data &
         (dpymp(i, 18), i=1, 2) &
         /1.414213562373095000000000000000E+00_dp, &
         5.816277809383742531415846947805E-26_dp/
      data &
         (dpymp(i, 19), i=1, 2) &
         /2.000624902374255782433465356007E+02_dp, &
         4.568236947482152283374593507328E+30_dp/
      data &
         (dpymp(i, 20), i=1, 2) &
         /2.000624902374255782433465356007E+02_dp, &
         1.848525209410256939008831977844E+05_dp/
      data &
         (dpymp(i, 21), i=1, 2) &
         /2.000624902374255782433465356007E+02_dp, &
         1.848525209410256939008831977844E+05_dp/
      data &
         (dpymp(i, 22), i=1, 2) &
         /2.731300056749532689792659000000E+00_dp, &
         3.378975642596100806258619000000E+05_dp/
      data &
         (dpymp(i, 23), i=1, 2) &
         /2.675757304209387399396291584708E+00_dp, &
         5.174484505019630309341494012187E-02_dp/
      data &
         (idpymp(i), i=1, 23) &
      ...
      test: do itest = 1, ntest
         ...
         elseif (itest == 17) then
            ...
            job = 00000
            maxit = 1000
            beta = [200.0_dp, 2.0_dp]
            lower = [10.0_dp, 0.0_dp]
            upper = [2.0_dp, 5.0_dp]
...
4200  format &
   ...
   !! Set up input data for odrpack exerciser.
      ...
      real(dp), intent(out), allocatable :: lower(:)
      real(dp), intent(out), allocatable :: upper(:)
      real(dp), intent(out), allocatable :: delta(:, :)
      integer, intent(out), allocatable :: ifixb(:)
      integer, intent(out), allocatable :: ifixx(:, :)
      real(dp), intent(out), allocatable :: wd(:, :, :)
      real(dp), intent(out), allocatable :: we(:, :, :)
      real(dp), intent(out), allocatable :: sclb(:)
      real(dp), intent(out), allocatable :: scld(:, :)
      real(dp), intent(out), allocatable :: stpb(:)
      real(dp), intent(out), allocatable :: stpd(:, :)
      ...
      ! Local arrays
      real(dp) :: bdata(maxnp, maxset), xdata(maxn, maxm, maxset), ydata(maxn, maxq, maxset)
      ...
      data &
         ...
         /-0.119569795672791172E+1_dp, -0.213701920211315155E-1_dp/
      data &
         ydata(2, 1, 1), xdata(2, 1, 1) &
         /-0.128023349509594288E+1_dp, 0.494813247025012969E-1_dp/
      data &
         ydata(3, 1, 1), xdata(3, 1, 1) &
         /-0.125270693343174591E+1_dp, 0.127889194935560226E+0_dp/
      data &
         ydata(4, 1, 1), xdata(4, 1, 1) &
         /-0.996698267935287383E+0_dp, 0.128615394085645676E+0_dp/
      data &
         ydata(5, 1, 1), xdata(5, 1, 1) &
         /-0.104681033065801934E+1_dp, 0.232544285655021667E+0_dp/
      data &
         ydata(6, 1, 1), xdata(6, 1, 1) &
         /-0.146724952092847308E+1_dp, 0.268151108026504516E+0_dp/
      data &
         ydata(7, 1, 1), xdata(7, 1, 1) &
         /-0.123366891873487528E+1_dp, 0.309041029810905456E+0_dp/
      data &
         ydata(8, 1, 1), xdata(8, 1, 1) &
         /-0.165665097907185554E+1_dp, 0.405991539210081099E+0_dp/
      data &
         ydata(9, 1, 1), xdata(9, 1, 1) &
         /-0.168476460930907119E+1_dp, 0.376611424833536147E+0_dp/
      data &
         ydata(10, 1, 1), xdata(10, 1, 1) &
         /-0.198571971169224491E+1_dp, 0.475875890851020811E+0_dp/
      data &
         ydata(11, 1, 1), xdata(11, 1, 1) &
         /-0.195691696638051344E+1_dp, 0.499246935397386550E+0_dp/
      data &
         ydata(12, 1, 1), xdata(12, 1, 1) &
         /-0.211871342665769836E+1_dp, 0.536615037024021147E+0_dp/
      data &
         ydata(13, 1, 1), xdata(13, 1, 1) &
         /-0.268642932558671020E+1_dp, 0.581830765902996060E+0_dp/
      data &
         ydata(14, 1, 1), xdata(14, 1, 1) &
         /-0.281123260058024347E+1_dp, 0.684512710422277446E+0_dp/
      data &
         ydata(15, 1, 1), xdata(15, 1, 1) &
         /-0.328704486581785920E+1_dp, 0.660219819694757458E+0_dp/
      data &
         ydata(16, 1, 1), xdata(16, 1, 1) &
         /-0.423062993461887032E+1_dp, 0.766990323960781092E+0_dp/
      data &
         ydata(17, 1, 1), xdata(17, 1, 1) &
         /-0.512043906552226903E+1_dp, 0.808270426690578456E+0_dp/
      data &
         ydata(18, 1, 1), xdata(18, 1, 1) &
         /-0.731032616379005535E+1_dp, 0.897410020083189004E+0_dp/
      data &
         ydata(19, 1, 1), xdata(19, 1, 1) &
         /-0.109002759485608993E+2_dp, 0.959199774116277687E+0_dp/
      data &
         ydata(20, 1, 1), xdata(20, 1, 1) &
         /-0.251810238510370206E+2_dp, 0.914675474762916558E+0_dp/
      data &
         ydata(21, 1, 1), xdata(21, 1, 1) &
         /0.100123028650879944E+3_dp, 0.997759691476821892E+0_dp/
      data &
         ydata(22, 1, 1), xdata(22, 1, 1) &
         /0.168225085871915048E+2_dp, 0.107136870384216308E+1_dp/
      data &
         ydata(23, 1, 1), xdata(23, 1, 1) &
         /0.894830510866913009E+1_dp, 0.108033321037888526E+1_dp/
      data &
         ydata(24, 1, 1), xdata(24, 1, 1) &
         /0.645853815227747004E+1_dp, 0.116064198672771453E+1_dp/
      data &
         ydata(25, 1, 1), xdata(25, 1, 1) &
         /0.498218564760117328E+1_dp, 0.119080889359116553E+1_dp/
      data &
         ydata(26, 1, 1), xdata(26, 1, 1) &
         /0.382971664718710476E+1_dp, 0.129418875187635420E+1_dp/
      data &
         ydata(27, 1, 1), xdata(27, 1, 1) &
         /0.344116492497344184E+1_dp, 0.135594148099422453E+1_dp/
      data &
         ydata(28, 1, 1), xdata(28, 1, 1) &
         /0.276840496973858949E+1_dp, 0.135302808716893195E+1_dp/
      data &
         ydata(29, 1, 1), xdata(29, 1, 1) &
         /0.259521665196956666E+1_dp, 0.137994666010141371E+1_dp/
      data &
         ydata(30, 1, 1), xdata(30, 1, 1) &
         /0.205996022794557661E+1_dp, 0.147630019545555113E+1_dp/
      data &
         ydata(31, 1, 1), xdata(31, 1, 1) &
         /0.197939614345337836E+1_dp, 0.153450708076357840E+1_dp/
      data &
         ydata(32, 1, 1), xdata(32, 1, 1) &
         /0.156739340562905589E+1_dp, 0.152805351451039313E+1_dp/
      data &
         ydata(33, 1, 1), xdata(33, 1, 1) &
         /0.159032057073028366E+1_dp, 0.157147316247224806E+1_dp/
      data &
         ydata(34, 1, 1), xdata(34, 1, 1) &
         /0.173102268158937949E+1_dp, 0.166649596005678175E+1_dp/
      data &
         ydata(35, 1, 1), xdata(35, 1, 1) &
         /0.155512561664824758E+1_dp, 0.166505665838718412E+1_dp/
      data &
         ydata(36, 1, 1), xdata(36, 1, 1) &
         /0.149635994944133260E+1_dp, 0.175214128553867338E+1_dp/
      data &
         ydata(37, 1, 1), xdata(37, 1, 1) &
         /0.147487601463073568E+1_dp, 0.180567992463707922E+1_dp/
      data &
         ydata(38, 1, 1), xdata(38, 1, 1) &
         /0.117244575233306998E+1_dp, 0.184624404296278952E+1_dp/
      data &
         ydata(39, 1, 1), xdata(39, 1, 1) &
         /0.910931336069172580E+0_dp, 0.195568727388978002E+1_dp/
      data &
         ydata(40, 1, 1), xdata(40, 1, 1) &
         /0.126172980914513272E+1_dp, 0.199326394036412237E+1_dp/
      ...
      data &
         (bdata(k, 2), k=1, 3) &
         /-1.0E+0_dp, 1.0E+0_dp, 1.0E+0_dp/
      data &
         ydata(1, 1, 2), xdata(1, 1, 2), xdata(1, 2, 2) &
         /0.680832777217942900E+0_dp, &
         0.625474598833994800E-1_dp, 0.110179064209783100E+0_dp/
      data &
         ydata(2, 1, 2), xdata(2, 1, 2), xdata(2, 2, 2) &
         /0.122183594595302200E+1_dp, &
         0.202500343620642400E+0_dp, -0.196140862891327600E-1_dp/
      data &
         ydata(3, 1, 2), xdata(3, 1, 2), xdata(3, 2, 2) &
         /0.118958678734608200E+1_dp, &
         0.164943738599876500E+0_dp, 0.166514874750996600E+0_dp/
      data &
         ydata(4, 1, 2), xdata(4, 1, 2), xdata(4, 2, 2) &
         /0.146982623764094600E+1_dp, &
         0.304874137610506100E+0_dp, 0.612908688041490500E-2_dp/
      data &
         ydata(5, 1, 2), xdata(5, 1, 2), xdata(5, 2, 2) &
         /0.167775338189355300E+1_dp, &
         0.532727445580665100E+0_dp, 0.938248787552444600E-1_dp/
      data &
         ydata(6, 1, 2), xdata(6, 1, 2), xdata(6, 2, 2) &
         /0.202485721906026200E+1_dp, &
         0.508823707598910200E+0_dp, 0.499605775020505400E-2_dp/
      data &
         ydata(7, 1, 2), xdata(7, 1, 2), xdata(7, 2, 2) &
         /0.258912851935938800E+1_dp, &
         0.704227041878554000E+0_dp, 0.819354849092326200E-1_dp/
      data &
         ydata(8, 1, 2), xdata(8, 1, 2), xdata(8, 2, 2) &
         /0.366894203254154800E+1_dp, &
         0.592077736111512000E+0_dp, 0.127113960672389100E-1_dp/
      data &
         ydata(9, 1, 2), xdata(9, 1, 2), xdata(9, 2, 2) &
         /0.574609583351347300E+1_dp, &
         0.104940945646421600E+1_dp, 0.258095243658316100E-1_dp/
      data &
         ydata(10, 1, 2), xdata(10, 1, 2), xdata(10, 2, 2) &
         /0.127676424026489300E+2_dp, &
         0.979382517558619200E+0_dp, 0.124280755181027900E+0_dp/
      data &
         ydata(11, 1, 2), xdata(11, 1, 2), xdata(11, 2, 2) &
         /0.123473079693623100E+1_dp, &
         0.637870453165538700E-1_dp, 0.304856401137196400E+0_dp/
      data &
         ydata(12, 1, 2), xdata(12, 1, 2), xdata(12, 2, 2) &
         /0.142256120864082800E+1_dp, &
         0.176123312906025700E+0_dp, 0.262387028078896900E+0_dp/
      data &
         ydata(13, 1, 2), xdata(13, 1, 2), xdata(13, 2, 2) &
         /0.169889534013024700E+1_dp, &
         0.310965082300263000E+0_dp, 0.226430765474758800E+0_dp/
      data &
         ydata(14, 1, 2), xdata(14, 1, 2), xdata(14, 2, 2) &
         /0.173485577901204400E+1_dp, &
         0.311394269116782100E+0_dp, 0.271375840410281800E+0_dp/
      data &
         ydata(15, 1, 2), xdata(15, 1, 2), xdata(15, 2, 2) &
         /0.277761263972834600E+1_dp, &
         0.447076126190612500E+0_dp, 0.255000858902618300E+0_dp/
      data &
         ydata(16, 1, 2), xdata(16, 1, 2), xdata(16, 2, 2) &
         /0.339163324662617300E+1_dp, &
         0.384786230998211100E+0_dp, 0.154958003178364000E+0_dp/
      data &
         ydata(17, 1, 2), xdata(17, 1, 2), xdata(17, 2, 2) &
         /0.589615137312147500E+1_dp, &
         0.649093176450780500E+0_dp, 0.258301685463773200E+0_dp/
      data &
         ydata(18, 1, 2), xdata(18, 1, 2), xdata(18, 2, 2) &
         /0.124415625214576800E+2_dp, &
         0.685612005372525500E+0_dp, 0.107391260603228600E+0_dp/
      data &
         ydata(19, 1, 2), xdata(19, 1, 2), xdata(19, 2, 2) &
         /-0.498491739153861600E+2_dp, &
         0.968747139425088400E+0_dp, 0.151932526135740700E+0_dp/
      data &
         ydata(20, 1, 2), xdata(20, 1, 2), xdata(20, 2, 2) &
         /-0.832795509000618600E+1_dp, &
         0.869789367989532900E+0_dp, 0.625507500586400000E-1_dp/
      data &
         ydata(21, 1, 2), xdata(21, 1, 2), xdata(21, 2, 2) &
         /0.184934617774239900E+1_dp, &
         -0.465309930332736600E-2_dp, 0.546795662595375200E+0_dp/
      data &
         ydata(22, 1, 2), xdata(22, 1, 2), xdata(22, 2, 2) &
         /0.175192979176839200E+1_dp, &
         0.604753397196646000E-2_dp, 0.230905749473922700E+0_dp/
      data &
         ydata(23, 1, 2), xdata(23, 1, 2), xdata(23, 2, 2) &
         /0.253949381238535800E+1_dp, &
         0.239418809621756000E+0_dp, 0.190752069681170700E+0_dp/
      data &
         ydata(24, 1, 2), xdata(24, 1, 2), xdata(24, 2, 2) &
         /0.373500774928501700E+1_dp, &
         0.456662468911699800E+0_dp, 0.328870615170984400E+0_dp/
      data &
         ydata(25, 1, 2), xdata(25, 1, 2), xdata(25, 2, 2) &
         /0.548408128950331000E+1_dp, &
         0.371115320522079500E+0_dp, 0.439978556640660500E+0_dp/
      data &
         ydata(26, 1, 2), xdata(26, 1, 2), xdata(26, 2, 2) &
         /0.125256880521774300E+2_dp, &
         0.586442107042503000E+0_dp, 0.490689043752286700E+0_dp/
      data &
         ydata(27, 1, 2), xdata(27, 1, 2), xdata(27, 2, 2) &
         /-0.493587797164916600E+2_dp, &
         0.579796274973298000E+0_dp, 0.521860998203383100E+0_dp/
      data &
         ydata(28, 1, 2), xdata(28, 1, 2), xdata(28, 2, 2) &
         /-0.801158974965412700E+1_dp, &
         0.805008094903899900E+0_dp, 0.292283538955391600E+0_dp/
      data &
         ydata(29, 1, 2), xdata(29, 1, 2), xdata(29, 2, 2) &
         /-0.437399487061934100E+1_dp, &
         0.637242340835710000E+0_dp, 0.402261740352486000E+0_dp/
      data &
         ydata(30, 1, 2), xdata(30, 1, 2), xdata(30, 2, 2) &
         /-0.297800103425979600E+1_dp, &
         0.982132817936118700E+0_dp, 0.392546836419047000E+0_dp/
      data &
         ydata(31, 1, 2), xdata(31, 1, 2), xdata(31, 2, 2) &
         /0.271811057454661300E+1_dp, &
         -0.223515657121262700E-1_dp, 0.650479019708978800E+0_dp/
      data &
         ydata(32, 1, 2), xdata(32, 1, 2), xdata(32, 2, 2) &
         /0.377035865613392400E+1_dp, &
         0.136081427545033600E+0_dp, 0.753020101897661800E+0_dp/
      data &
         ydata(33, 1, 2), xdata(33, 1, 2), xdata(33, 2, 2) &
         /0.560111053917143100E+1_dp, &
         0.145367053019870600E+0_dp, 0.611153532003093100E+0_dp/
      data &
         ydata(34, 1, 2), xdata(34, 1, 2), xdata(34, 2, 2) &
         /0.128152376174926800E+2_dp, &
         0.308221919576435500E+0_dp, 0.455217283290423900E+0_dp/
      data &
         ydata(35, 1, 2), xdata(35, 1, 2), xdata(35, 2, 2) &
         /-0.498709177732467200E+2_dp, &
         0.432658769133528300E+0_dp, 0.678607663414113000E+0_dp/
      data &
         ydata(36, 1, 2), xdata(36, 1, 2), xdata(36, 2, 2) &
         /-0.815797696908314300E+1_dp, &
         0.477785501079980300E+0_dp, 0.536178207572157000E+0_dp/
      data &
         ydata(37, 1, 2), xdata(37, 1, 2), xdata(37, 2, 2) &
         /-0.440240491195158600E+1_dp, &
         0.727986827616619000E+0_dp, 0.668497920573493900E+0_dp/
      data &
         ydata(38, 1, 2), xdata(38, 1, 2), xdata(38, 2, 2) &
         /-0.276723957061767500E+1_dp, &
         0.745950385588265100E+0_dp, 0.786077589007263700E+0_dp/
      data &
         ydata(39, 1, 2), xdata(39, 1, 2), xdata(39, 2, 2) &
         /-0.223203667288734800E+1_dp, &
         0.732537503527113500E+0_dp, 0.582625164046828400E+0_dp/
      data &
         ydata(40, 1, 2), xdata(40, 1, 2), xdata(40, 2, 2) &
         /-0.169728270310622000E+1_dp, &
         0.967352361433846300E+0_dp, 0.460779396016832800E+0_dp/
      data &
         ydata(41, 1, 2), xdata(41, 1, 2), xdata(41, 2, 2) &
         /0.551015652153227000E+1_dp, &
         0.129761784310891100E-1_dp, 0.700009537931860000E+0_dp/
      data &
         ydata(42, 1, 2), xdata(42, 1, 2), xdata(42, 2, 2) &
         /0.128036180496215800E+2_dp, &
         0.170163243950629700E+0_dp, 0.853131830764348700E+0_dp/
      data &
         ydata(43, 1, 2), xdata(43, 1, 2), xdata(43, 2, 2) &
         /-0.498257683396339000E+2_dp, &
         0.162768461906274000E+0_dp, 0.865315129048175000E+0_dp/
      data &
         ydata(44, 1, 2), xdata(44, 1, 2), xdata(44, 2, 2) &
         /-0.877334550221761900E+1_dp, &
         0.222914807946165800E+0_dp, 0.797511758502094500E+0_dp/
      data &
         ydata(45, 1, 2), xdata(45, 1, 2), xdata(45, 2, 2) &
         /-0.453820192156867600E+1_dp, &
         0.402910095604624900E+0_dp, 0.761492958727023100E+0_dp/
      data &
         ydata(46, 1, 2), xdata(46, 1, 2), xdata(46, 2, 2) &
         /-0.297499315738677900E+1_dp, &
         0.233770812593443200E+0_dp, 0.896000095844223500E+0_dp/
      data &
         ydata(47, 1, 2), xdata(47, 1, 2), xdata(47, 2, 2) &
         /-0.212743255978538900E+1_dp, &
         0.646528693486914700E+0_dp, 0.968574333700755700E+0_dp/
      data &
         ydata(48, 1, 2), xdata(48, 1, 2), xdata(48, 2, 2) &
         /-0.209703205365401000E+1_dp, &
         0.802811658568969400E+0_dp, 0.904866450476711600E+0_dp/
      data &
         ydata(49, 1, 2), xdata(49, 1, 2), xdata(49, 2, 2) &
         /-0.155287292042086200E+1_dp, &
         0.837137859891222900E+0_dp, 0.835684424990021900E+0_dp/
      data &
         ydata(50, 1, 2), xdata(50, 1, 2), xdata(50, 2, 2) &
         /-0.161356673770480700E+1_dp, &
         0.103165980756526600E+1_dp, 0.793902191912346100E+0_dp/
      ...
      data &
         (bdata(k, 3), k=1, 9) &
         /0.281887509408440189E-5_dp, &
         -0.231290549212363845E-2_dp, 0.583035555572801965E+1_dp, &
         0.000000000000000000E+0_dp, 0.406910776203121026E+8_dp, &
         0.138001105225000000E-2_dp, 0.596038513209999999E-1_dp, &
         0.670582099359999998E+1_dp, 0.106994410000000000E+10_dp/
      data &
         ydata(1, 1, 3), xdata(1, 1, 3) &
         /0.988227696721327788E+0_dp, 0.25E-8_dp/
      data &
         ydata(2, 1, 3), xdata(2, 1, 3) &
         /0.988268083998559958E+0_dp, 0.64E-8_dp/
      data &
         ydata(3, 1, 3), xdata(3, 1, 3) &
         /0.988341022958438831E+0_dp, 1.0E-8_dp/
      data &
         ydata(4, 1, 3), xdata(4, 1, 3) &
         /0.988380557606306446E+0_dp, 0.9E-7_dp/
      data &
         ydata(5, 1, 3), xdata(5, 1, 3) &
         /0.988275062411751338E+0_dp, 1.0E-6_dp/
      data &
         ydata(6, 1, 3), xdata(6, 1, 3) &
         /0.988326680176446987E+0_dp, 0.4E-5_dp/
      data &
         ydata(7, 1, 3), xdata(7, 1, 3) &
         /0.988306058860433439E+0_dp, 0.9E-5_dp/
      data &
         ydata(8, 1, 3), xdata(8, 1, 3) &
         /0.988292880079125555E+0_dp, 0.16E-4_dp/
      data &
         ydata(9, 1, 3), xdata(9, 1, 3) &
         /0.988305279259496905E+0_dp, 0.36E-4_dp/
      data &
         ydata(10, 1, 3), xdata(10, 1, 3) &
         /0.988278142019574202E+0_dp, 0.64E-4_dp/
      data &
         ydata(11, 1, 3), xdata(11, 1, 3) &
         /0.988224953369819946E+0_dp, 1.0E-4_dp/
      data &
         ydata(12, 1, 3), xdata(12, 1, 3) &
         /0.988111989169778223E+0_dp, 0.144E-3_dp/
      data &
         ydata(13, 1, 3), xdata(13, 1, 3) &
         /0.988045627103840613E+0_dp, 0.225E-3_dp/
      data &
         ydata(14, 1, 3), xdata(14, 1, 3) &
         /0.987913715667047655E+0_dp, 0.400E-3_dp/
      data &
         ydata(15, 1, 3), xdata(15, 1, 3) &
         /0.987841994238525678E+0_dp, 0.625E-3_dp/
      data &
         ydata(16, 1, 3), xdata(16, 1, 3) &
         /0.987638450432434270E+0_dp, 0.900E-3_dp/
      data &
         ydata(17, 1, 3), xdata(17, 1, 3) &
         /0.987587364331771395E+0_dp, 0.1225E-2_dp/
      data &
         ydata(18, 1, 3), xdata(18, 1, 3) &
         /0.987576264149633684E+0_dp, 0.1600E-2_dp/
      data &
         ydata(19, 1, 3), xdata(19, 1, 3) &
         /0.987539209110983643E+0_dp, 0.2025E-2_dp/
      data &
         ydata(20, 1, 3), xdata(20, 1, 3) &
         /0.987621143807705698E+0_dp, 0.25E-2_dp/
      data &
         ydata(21, 1, 3), xdata(21, 1, 3) &
         /0.988023229785526217E+0_dp, 0.36E-2_dp/
      data &
         ydata(22, 1, 3), xdata(22, 1, 3) &
         /0.988558376710994197E+0_dp, 0.49E-2_dp/
      data &
         ydata(23, 1, 3), xdata(23, 1, 3) &
         /0.989304775352439885E+0_dp, 0.64E-2_dp/
      data &
         ydata(24, 1, 3), xdata(24, 1, 3) &
         /0.990210452265710472E+0_dp, 0.81E-2_dp/
      data &
         ydata(25, 1, 3), xdata(25, 1, 3) &
         /0.991095950592263900E+0_dp, 1.00E-2_dp/
      data &
         ydata(26, 1, 3), xdata(26, 1, 3) &
         /0.991475677297119272E+0_dp, 0.11025E-1_dp/
      data &
         ydata(27, 1, 3), xdata(27, 1, 3) &
         /0.991901306250746771E+0_dp, 0.12100E-1_dp/
      data &
         ydata(28, 1, 3), xdata(28, 1, 3) &
         /0.992619222425303263E+0_dp, 0.14400E-1_dp/
      data &
         ydata(29, 1, 3), xdata(29, 1, 3) &
         /0.993617037631973475E+0_dp, 0.16900E-1_dp/
      data &
         ydata(30, 1, 3), xdata(30, 1, 3) &
         /0.994727321698030676E+0_dp, 0.19600E-1_dp/
      data &
         ydata(31, 1, 3), xdata(31, 1, 3) &
         /0.996523114720326189E+0_dp, 0.25600E-1_dp/
      data &
         ydata(32, 1, 3), xdata(32, 1, 3) &
         /0.998036909563764020E+0_dp, 0.32400E-1_dp/
      data &
         ydata(33, 1, 3), xdata(33, 1, 3) &
         /0.999151968626971372E+0_dp, 0.40000E-1_dp/
      data &
         ydata(34, 1, 3), xdata(34, 1, 3) &
         /0.100017083706131769E+1_dp, 0.50625E-1_dp/
      data &
         ydata(35, 1, 3), xdata(35, 1, 3) &
         /0.100110046382923523E+1_dp, 0.75625E-1_dp/
      data &
         ydata(36, 1, 3), xdata(36, 1, 3) &
         /0.100059103180404652E+1_dp, 0.12250E+0_dp/
      data &
         ydata(37, 1, 3), xdata(37, 1, 3) &
         /0.999211829791257561E+0_dp, 0.16000E+0_dp/
      data &
         ydata(38, 1, 3), xdata(38, 1, 3) &
         /0.994711451526761862E+0_dp, 0.25000E+0_dp/
      data &
         ydata(39, 1, 3), xdata(39, 1, 3) &
         /0.989844132928847109E+0_dp, 0.33640E+0_dp/
      data &
         ydata(40, 1, 3), xdata(40, 1, 3) &
         /0.987234104554490439E+0_dp, 0.38440E+0_dp/
      data &
         ydata(41, 1, 3), xdata(41, 1, 3) &
         /0.980928240178404887E+0_dp, 0.49E+0_dp/
      data &
         ydata(42, 1, 3), xdata(42, 1, 3) &
         /0.970888680366055576E+0_dp, 0.64E+0_dp/
      data &
         ydata(43, 1, 3), xdata(43, 1, 3) &
         /0.960043769857327398E+0_dp, 0.81E+0_dp/
      data &
         ydata(44, 1, 3), xdata(44, 1, 3) &
         /0.947277159259551068E+0_dp, 1.00E+0_dp/
      ...
      data &
         (bdata(k, 5), k=1, 2) &
         /0.01155E+0_dp, 5000.0E+0_dp/
      data &
         ydata(1, 1, 5), xdata(1, 1, 5), xdata(1, 2, 5) &
         /0.912E+0_dp, 109.0E+0_dp, 600.0E+0_dp/
      data &
         ydata(2, 1, 5), xdata(2, 1, 5), xdata(2, 2, 5) &
         /0.382E+0_dp, 65.0E+0_dp, 640.0E+0_dp/
      data &
         ydata(3, 1, 5), xdata(3, 1, 5), xdata(3, 2, 5) &
         /0.397E+0_dp, 1180.0E+0_dp, 600.0E+0_dp/
      data &
         ydata(4, 1, 5), xdata(4, 1, 5), xdata(4, 2, 5) &
         /0.376E+0_dp, 66.0E+0_dp, 640.0E+0_dp/
      data &
         ydata(5, 1, 5), xdata(5, 1, 5), xdata(5, 2, 5) &
         /0.342E+0_dp, 1270.0E+0_dp, 600.0E+0_dp/
      data &
         ydata(6, 1, 5), xdata(6, 1, 5), xdata(6, 2, 5) &
         /0.358E+0_dp, 69.0E+0_dp, 640.0E+0_dp/
      data &
         ydata(7, 1, 5), xdata(7, 1, 5), xdata(7, 2, 5) &
         /0.348E+0_dp, 1230.0E+0_dp, 600.0E+0_dp/
      data &
         ydata(8, 1, 5), xdata(8, 1, 5), xdata(8, 2, 5) &
         /0.376E+0_dp, 68.0E+0_dp, 640.0E+0_dp/
      ...
      data &
         (bdata(k, 8), k=1, 5) &
         /4.0E+0_dp, 2.0E+0_dp, 7.0E+0_dp, 0.40E+0_dp, 0.50E+0_dp/
      data &
         ydata(1, 1, 8), ydata(1, 2, 8), xdata(1, 1, 8) &
         /4.220E+0_dp, 0.136E+0_dp, 30.0E+0_dp/
      data &
         ydata(2, 1, 8), ydata(2, 2, 8), xdata(2, 1, 8) &
         /4.167E+0_dp, 0.167E+0_dp, 50.0E+0_dp/
      data &
         ydata(3, 1, 8), ydata(3, 2, 8), xdata(3, 1, 8) &
         /4.132E+0_dp, 0.188E+0_dp, 70.0E+0_dp/
      data &
         ydata(4, 1, 8), ydata(4, 2, 8), xdata(4, 1, 8) &
         /4.038E+0_dp, 0.212E+0_dp, 100.0E+0_dp/
      data &
         ydata(5, 1, 8), ydata(5, 2, 8), xdata(5, 1, 8) &
         /4.019E+0_dp, 0.236E+0_dp, 150.0E+0_dp/
      data &
         ydata(6, 1, 8), ydata(6, 2, 8), xdata(6, 1, 8) &
         /3.956E+0_dp, 0.257E+0_dp, 200.0E+0_dp/
      data &
         ydata(7, 1, 8), ydata(7, 2, 8), xdata(7, 1, 8) &
         /3.884E+0_dp, 0.276E+0_dp, 300.0E+0_dp/
      data &
         ydata(8, 1, 8), ydata(8, 2, 8), xdata(8, 1, 8) &
         /3.784E+0_dp, 0.297E+0_dp, 500.0E+0_dp/
      data &
         ydata(9, 1, 8), ydata(9, 2, 8), xdata(9, 1, 8) &
         /3.713E+0_dp, 0.309E+0_dp, 700.0E+0_dp/
      data &
         ydata(10, 1, 8), ydata(10, 2, 8), xdata(10, 1, 8) &
         /3.633E+0_dp, 0.311E+0_dp, 1000.0E+0_dp/
      data &
         ydata(11, 1, 8), ydata(11, 2, 8), xdata(11, 1, 8) &
         /3.540E+0_dp, 0.314E+0_dp, 1500.0E+0_dp/
      data &
         ydata(12, 1, 8), ydata(12, 2, 8), xdata(12, 1, 8) &
         /3.433E+0_dp, 0.311E+0_dp, 2000.0E+0_dp/
      data &
         ydata(13, 1, 8), ydata(13, 2, 8), xdata(13, 1, 8) &
         /3.358E+0_dp, 0.305E+0_dp, 3000.0E+0_dp/
      data &
         ydata(14, 1, 8), ydata(14, 2, 8), xdata(14, 1, 8) &
         /3.258E+0_dp, 0.289E+0_dp, 5000.0E+0_dp/
      data &
         ydata(15, 1, 8), ydata(15, 2, 8), xdata(15, 1, 8) &
         /3.193E+0_dp, 0.277E+0_dp, 7000.0E+0_dp/
      data &
         ydata(16, 1, 8), ydata(16, 2, 8), xdata(16, 1, 8) &
         /3.128E+0_dp, 0.255E+0_dp, 10000.0E+0_dp/
      data &
         ydata(17, 1, 8), ydata(17, 2, 8), xdata(17, 1, 8) &
         /3.059E+0_dp, 0.240E+0_dp, 15000.0E+0_dp/
      data &
         ydata(18, 1, 8), ydata(18, 2, 8), xdata(18, 1, 8) &
         /2.984E+0_dp, 0.218E+0_dp, 20000.0E+0_dp/
      data &
         ydata(19, 1, 8), ydata(19, 2, 8), xdata(19, 1, 8) &
         /2.934E+0_dp, 0.202E+0_dp, 30000.0E+0_dp/
      data &
         ydata(20, 1, 8), ydata(20, 2, 8), xdata(20, 1, 8) &
         /2.876E+0_dp, 0.182E+0_dp, 50000.0E+0_dp/
      data &
         ydata(21, 1, 8), ydata(21, 2, 8), xdata(21, 1, 8) &
         /2.838E+0_dp, 0.168E+0_dp, 70000.0E+0_dp/
      data &
         ydata(22, 1, 8), ydata(22, 2, 8), xdata(22, 1, 8) &
         /2.798E+0_dp, 0.153E+0_dp, 100000.0E+0_dp/
      data &
         ydata(23, 1, 8), ydata(23, 2, 8), xdata(23, 1, 8) &
         /2.759E+0_dp, 0.139E+0_dp, 150000.0E+0_dp/
      ...
      data &
         ...
         /2.718281828459045_dp, 1.0_dp/
      data &
         ydata(2, 1, 9), xdata(2, 1, 9) &
         /7.389056098930650_dp, 2.0_dp/
      data &
         ydata(3, 1, 9), xdata(3, 1, 9) &
         /148.4131591025766_dp, 5.0_dp/
      data &
         ydata(4, 1, 9), xdata(4, 1, 9) &
         /403.4287934927353_dp, 6.0_dp/
   ...
   !! Compute model function and jacobian for odrpack exerciser.
      ...
      ! Local scalars
      real(dp) :: ctheta, fac1, fac2, fac3, fac4, freq, omega, phi, pi, r, stheta, theta
      integer :: i, j, k
         ...
         if (mod(ideval, 10) /= 0) then
            do i = 1, n
               f(i, 1) = exp(-beta(1)*xplusd(i, 1)* &
                             exp(-beta(2)*(1/xplusd(i, 2) - 1/620.0E0_dp)))
            end do
         end if

         if (mod(ideval/10, 10) /= 0) then
            do i = 1, n
               fac1 = 1/xplusd(i, 2) - 1/620.0E0_dp
         ...
         if (mod(ideval, 10) /= 0) then
            do i = 1, n
               f(i, 1) = beta(3)*(xplusd(i, 1) - beta(1))**2 + &
                         ...
                         (xplusd(i, 2) - beta(2)) + &
                         beta(5)*(xplusd(i, 2) - beta(2))**2 - 1.0E0_dp
            end do
         end if
      ...
      elseif (setno == 8) then
         ...
         if (mod(ideval, 10) /= 0) then
            pi = 3.141592653589793238462643383279E0_dp
            theta = pi*beta(4)*0.5E0_dp
            ctheta = cos(theta)
            stheta = sin(theta)
            do i = 1, n
               freq = xplusd(i, 1)
               omega = (2.0E0_dp*pi*freq*exp(-beta(3)))**beta(4)
      ...
      elseif (setno == 10) then
         ...
         if (mod(ideval, 10) /= 0) then
            do i = 1, n
               f(i, 1) = beta(1)/2.0_dp*exp(beta(2)*xplusd(i, 1))
            end do
         end if

         if (mod(ideval/10, 10) /= 0) then
            do i = 1, n
               fjacb(i, 1, 1) = exp(beta(2)*xplusd(i, 1))/2.0_dp
               fjacb(i, 2, 1) = beta(1)/2.0_dp*xplusd(i, 1)*exp(beta(2)*xplusd(i, 1))
            end do
         end if

         if (mod(ideval/100, 10) /= 0) then
            do i = 1, n
               fjacd(i, 1, 1) = beta(1)/2.0_dp*beta(2)*exp(beta(2)*xplusd(i, 1))
            end do
         end if
